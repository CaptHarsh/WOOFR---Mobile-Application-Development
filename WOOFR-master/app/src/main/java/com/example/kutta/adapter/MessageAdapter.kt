...
class MessageAdapter(val context: Context , val list: List<MessageModel>):RecyclerView.Adapter<MessageAdapter.MessageViewHolder>() {
    ...
    override fun getItemViewType(position: Int): Int {
        return if(list[position].senderId1==FirebaseAuth.getInstance().currentUser!!.phoneNumber){
            MSG_TYPE_RIGHT
        }else MSG_TYPE_LEFT
    ...
    override fun onBindViewHolder(holder: MessageViewHolder, position: Int) {
        ...
        FirebaseDatabase.getInstance().getReference("users")
            .child(list[position].senderId1!!).addListenerForSingleValueEvent(
                object  : ValueEventListener {
                    ...
                    override fun onDataChange(snapshot: DataSnapshot) {
                        if(snapshot.exists()){
                            ...
                            Glide.with(context).load(data!!.image).placeholder(R.drawable.profile_dog).into(holder.image)
