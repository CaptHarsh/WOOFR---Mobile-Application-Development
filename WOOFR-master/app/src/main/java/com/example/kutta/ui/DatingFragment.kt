...
import com.example.kutta.adapter.DatingAdapter
import com.example.kutta.databinding.FragmentDatingBinding
        ...
        manager= CardStackLayoutManager(requireContext(),object :CardStackListener{
            ...
            override fun onCardSwiped(direction: Direction?) {
                    if(manager.topPosition==list!!.size){
                        Toast.makeText(requireContext(),"This is last ",Toast.LENGTH_SHORT).show()
                    }
            }
        ...
        FirebaseDatabase.getInstance().getReference("users")
            .addValueEventListener(object : ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) {
                    Log.d("Harsh","onDataChange : ${snapshot.toString()}")
                    if(snapshot.exists()){
                         list = arrayListOf()
